...
def run_slam(src_dir, log_dir, idx, split):
    ...
    xyth_init = slam.lidar[t0]['xyth']
    xyth_init[2] = slam.lidar[t0]['rpy'][2]
        
    #### TODO: XXXXXXXXXXX
    
    #do a 100 particles
    slam.init_particles(n=1,p=xyth_init.reshape((3,1)),w=np.array([1]))
    slam.observation_step(t=0)
    ...
    for i in tqdm.tqdm(range(t0,T)):
        slam.dynamics_step(i)
        slam.observation_step(i)
        ind = np.argmax(slam.w)
        psx.append(slam.p[:, ind][0])
        psy.append(slam.p[:, ind][1])
        ps = np.hstack((ps, slam.p))

        if(i % 100 == 0 and i>20):
    
            plt.title('Particles %03d'%i)
            ...
            plt.plot(some_odom[:,1], some_odom[:,0],'.c', markersize = 5 ) 
            plt.plot(some_arr[:,1], some_arr[:,0],'.r', markersize = 5 ) 
